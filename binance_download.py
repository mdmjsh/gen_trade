...
DF_COLUMNS = ['open_ts', 'open', 'high', 'low',
              'close', 'volume', 'close_ts', 'qav', 'num_trades']
...
def main(start_date: str, end_date: str, symbol_a: str, symbol_b: str, csv_path: str) -> None:
    client = Client(os.environ["BINANCE_API"], os.environ["BINANCE_SECRET"])
    klines = client.get_historical_klines(
        f"{symbol_a}{symbol_b}", client.KLINE_INTERVAL_15MINUTE, start_date, end_date)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_date', type=str,
                        help="Start date - human readable (see Binance API docs) - default `1 Jan, 2018`", required=False)
    parser.add_argument('--end_date', type=str,
                        help="- human readable (see Binance API docs) default `1 day ago UTC`", required=False)
    parser.add_argument('--symbol_a', type=str,
                        help='First ticker in the trade - default BTC', required=False)
    parser.add_argument('--symbol_b', type=str,
                        help='Secondary ticker in the trade - default USDC', required=False)
    parser.add_argument('--csv_path', type=str,
                        help='Output path/filename for CSV. If None uses the concat of the symbols.', required=False)

    parser.set_defaults(start_date="1 Jan, 2018",
                        end_date="1 day ago UTC", symbol_a="BTC", symbol_b="USDC")
