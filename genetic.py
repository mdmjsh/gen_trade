...
# @profile
def main(trading_data: pd.DataFrame,
...
def fitness_function_ha_and_moon(strategy: pd.DataFrame) -> pd.DataFrame:
    ...
    strategy['fitness'] = strategy.apply(lambda x: log((x.close + 17556) /
                                                       x.close), axis=1)
    return pd.DataFrame([{
...
def fitness_function(strategy: pd.DataFrame) -> pd.DataFrame:
    """ Win percentage * number of trades gives a performance coeffient.
...
def fitness_metadata(strategy):
    try:
        ...
        win_percent = (len(strategy.loc[strategy.result == 'TARGET_HIT']) /
                       n_trades) * 100
...
def apply_ranking(results: Tuple) -> Tuple[pd.DataFrame, Dict]:
    ...
    weights = {
        df.iloc[i].strategy['id']: n_items + 1 / (i + 1)
...
def select_parents(ranked: pd.DataFrame,
                   _weights: Dict,
                   parents: Dict = {}) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def cross_over_pmx(strat_x: Dict, strat_y: Dict) -> Tuple[Dict, Dict]:
    ...
    pmx_cross_over_range = min(max(len(child_x_inds) - 1, 0),
                               max(len(child_y_inds) - 1, 0))
...
def mutate(strategy: List):
    ...
    if abs_strats:
        ...
        mutate_ind['abs_value'] = mutate_ind['abs_value'] * (1 +
                                                             mutate_percent)

    return _strat


def is_profitable(max_profit: float, max_loss: float,
                  high_timestamp: pd.Timestamp,
                  low_timestamp: pd.Timestamp) -> bool:
...
def is_loss_making(max_profit: float, max_loss: float,
                   high_timestamp: pd.Timestamp,
                   low_timestamp: pd.Timestamp) -> bool:
...
if __name__ == "__main__":
    ...
    start = arrow.utcnow()
    results = main(load_trading_data(),
                   strategies,
